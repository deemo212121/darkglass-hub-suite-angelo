-- Start Date for a newly-hired candidate — captured the same way
-- interview_date/training_start_date/withdrawn_date already are, via the
-- "Hired" status dialog. Reuses hr_update_candidate_status()'s existing
-- generic p_effective_date param (no new RPC parameter needed) — see
-- ReportHRDaily.tsx's STATUS_REQUIRES_DATE.
--
-- Run once in the Supabase SQL Editor, after 0260.

alter table hr_candidates add column if not exists start_date date;
