...
export async function getPartsToReceive(): Promise<PartReceiveRow[]> {
  const { data, error } = await supabase
    .from("parts")
    .select(
      "id, po_no, po_date, order_no, invoice_no, note, part_no, part_desc, eta, received_date, in_tracking, part_dist, quantity, qty_received, part_price, core_value, tickets!inner(ticket_no, technician, status, location, schedule_date)"
    )
    .eq("status", "PO Made");
