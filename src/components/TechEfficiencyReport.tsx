...
export function TechEfficiencyReport({ mod, sub }: Props) {
  ...
  const [applied, setApplied] = useState({ location:"", startDate:offsetStr(-14), endDate:offsetStr(-1) });

  const rows = useMemo(() => {
    let r = ALL_ROWS;
    if (applied.location) r = r.filter(x=>x.location===applied.location);
    if (applied.startDate) r = r.filter(x=>x.schedDate>=applied.startDate);
    if (applied.endDate) r = r.filter(x=>x.schedDate<=applied.endDate);
    return r;
  }, [applied]);
