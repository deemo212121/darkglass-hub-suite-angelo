...
export function TaxReport({ mod, sub }: Props) {
  ...
  const [applied, setApplied] = useState({ location:"", startDate:offsetStr(-14), endDate:todayStr(), includeTaxProcessed:false });

  const rows = useMemo(() => {
    let r = ALL_ROWS;
    if (applied.location) r = r.filter(x=>x.location===applied.location);
    if (applied.startDate) r = r.filter(x=>x.claimDate>=applied.startDate);
    if (applied.endDate) r = r.filter(x=>x.claimDate<=applied.endDate);
    if (!applied.includeTaxProcessed) r = r.filter(x=>!x.taxProcessed);
    return r;
  }, [applied]);
    ...
    <main className="max-w-350 mx-auto px-4 py-6">
      ...
      <div className="panel mb-5">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={()=>setApplied({location,startDate,endDate,includeTaxProcessed})} className="btn btn-primary flex items-center gap-2 px-5">
