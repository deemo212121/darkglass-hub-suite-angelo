...
function generateRows(count = 40) {
...
const ALL_ROWS = generateRows(40);
...
export function ClaimPlanner({ mod, sub }: Props) {
  ...
  const [applied, setApplied] = useState({ location:"", statuses:["Completed"], date:todayStr() });
  ...
  const handleShow = () => {
    const statuses = [statusCompleted&&"Completed",statusOpen&&"Open",statusPending&&"Pending"].filter(Boolean) as string[];
    setApplied({location,statuses,date});
  };
    ...
    <main className="max-w-350 mx-auto px-4 py-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="flex items-center gap-1.5 text-sm cursor-pointer">
              <input type="checkbox" checked={statusCompleted} onChange={e=>setStatusCompleted(e.target.checked)} className="accent-blue-500" title="Completed"/>
              Completed
            </label>
            <label className="flex items-center gap-1.5 text-sm cursor-pointer">
              <input type="checkbox" checked={statusOpen} onChange={e=>setStatusOpen(e.target.checked)} className="accent-blue-500" title="Open"/>
              Open
            </label>
            <label className="flex items-center gap-1.5 text-sm cursor-pointer">
              <input type="checkbox" checked={statusPending} onChange={e=>setStatusPending(e.target.checked)} className="accent-blue-500" title="Pending"/>
              Pending
            </label>
          </div>
          <button onClick={handleShow} className="btn btn-primary px-5 text-sm">Show</button>
        </div>
      </div>
      ...
      <div className="panel mb-4">
        <div className="flex items-center justify-center gap-3">
          ...
          <label htmlFor="planner-date" className="sr-only">Select date</label>
          <input id="planner-date" type="date" value={date} onChange={e=>setDate(e.target.value)} title="Planner date" className="glass-input text-sm py-1.5 px-3 rounded-md w-40 text-center"/>
      ...
      <div className="panel overflow-x-auto p-0">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length===0
              ? <tr><td colSpan={9} className="px-4 py-12 text-center text-muted-foreground">No records match the current filters.</td></tr>
              : rows.map((r,idx)=>(
